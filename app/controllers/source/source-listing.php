...
$page_count = ceil(Source::count() / $page_size);
...
$source_query = Source::select()->limit($page_size)->offset($offset);
$source_alias_query = SourceAlias::select()->where("source_id IN (?)", $source_query->pluck("id"));

$sources = array();
foreach ($source_query->each() as $source)
{
    $sources[$source->id] = $source;
}

$source_aliases = array();
foreach ($source_alias_query->each() as $alias)
{
